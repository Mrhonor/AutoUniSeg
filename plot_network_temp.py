...
cv_lb = ["Sky", "Building", "Column_Pole", "Road", "Sidewalk", "Tree", "SignSymbol", "Fence", "Car", "Pedestrian", "Bicyclist"]
...
# print(id_to_ori_id)
# state = torch.load('output/find_unuse_7ds_model_hrnet.pth')
# state = torch.load('output/model_0054999.pth')
# state = torch.load('output/model_0004999.pth')
# bigraphs = [state()[f'proj_head.bipartite_graphs.{i}'] for i in range(len(datasets))]
# bigraphs = [state[f'proj_head.bipartite_graphs.{i}'] for i in range(len(datasets))]
...
with open('output/gt_relation_graph_7ds.pickle', "rb") as file:
...
ID_list = []
myvalue_list = []
this_graph = relation_gt_graph[0][1]
for i in range(this_graph.shape[0]):
    for j in range(this_graph.shape[1]):
        if this_graph[i][j] == 1:
            from_list.append(city_lb[i])
            to_list.append(mapi_lb[j])

            # print(len(colors[ds]))
            
            # colors[datasets[1]][i] = palette[j]
ID_list.extend(city_lb)
myvalue_list.extend(['city']*len(city_lb))
ID_list.extend(mapi_lb)
myvalue_list.extend(['mapi']*len(mapi_lb))
...
plt.savefig("Graph_relation.png", format="PNG")