...
from auto_uni_seg.utils.evaluate import eval_link_hook, iter_info_hook, find_unuse_hook
...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        elif cfg.INPUT.DATASET_MAPPER_NAME == 'PANO':
            # return DaLiLoaderAdapter(cfg, aux_mode='train')
            return PanoLoaderAdapter(cfg, aux_mode='train')
        else:
            mapper = None
            return build_detection_train_loader(cfg, mapper=mapper)
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        for ds_name in datasets:
            if dataset_name in ds_name:
                break
            dataset_id += 1
        if dataset_id == len(datasets):
            raise ValueError(f"dataset_name:{dataset_name} not found in cfg.DATASETS.TEST")
        ...
        if cfg.INPUT.DATASET_MAPPER_NAME == 'DALI':
            return LoaderAdapter(cfg, aux_mode=aux_mode, dataset_id=dataset_id)
        elif cfg.INPUT.DATASET_MAPPER_NAME == 'PANO':
            return PanoLoaderAdapter(cfg, aux_mode=aux_mode, dataset_id=dataset_id)
